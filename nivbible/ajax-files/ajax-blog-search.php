...
query_posts(array('post_type'=>'post','s'=>$searchval,'posts_per_page'=>$per_page,'paged'=>$paged));
      ...
      if($featured_post=='1'){
       ...
       //$html.='</div>';
		...
		$totalPost = query_posts('s='.$searchval.'&posts_per_page=-1&post_type=post');
...
/* ----------------------------------------------------------------------------------------------------------- */
	...
	for ($i = $start_loop; $i <= $end_loop; $i++) {
		...
		if ($cur_page == $i)
			$phtml .= "<li p='$i'class='active current' datasearch='$searchval' datacat='$category_id'>{$i}</li>";
		else
			$phtml .= "<li p='$i' class='active' datasearch='$searchval' datacat='$category_id'>{$i}</li>";
	}
